...
5. **Supports border-radius and border-width as distinct types** — Proper matching for radius and stroke tokens

### Path-Based Matching Helpers

Added helper methods to infer token type from path:
- `looksLikeSpacingToken()` — Checks for keywords: spacing, space, size, sizing, dimension, width, height, padding, margin, gap, radius, border, inset, offset, and numeric patterns like "1x", "2x"
- `looksLikeBorderRadiusToken()` — Checks for keywords: radius, corner, round, rounded, borderradius
- `looksLikeBorderToken()` — Checks for keywords: border, stroke, outline, line (excluding radius)
- `looksLikeEffectToken()` — Checks for keywords: shadow, effect, blur, elevation, drop

---

## Comprehensive Token Key Reference (January 2026)

### Keys Checked by Property Type

**Spacing/Dimensions:**
| Property | Keys Checked (in order of priority) |
|----------|-------------------------------------|
...
| gap/itemSpacing | `itemSpacing`, `spacing`, `gap`, `counterAxisSpacing` |

**Border Radius:**
| Property | Keys Checked |
|----------|-------------|
| borderRadius | `borderRadius`, `borderRadiusTopLeft`, `borderRadiusTopRight`, `borderRadiusBottomRight`, `borderRadiusBottomLeft`, `radius`, `cornerRadius`, `topLeftRadius`, `topRightRadius`, `bottomRightRadius`, `bottomLeftRadius` |

**Border Width:**
| Property | Keys Checked |
|----------|-------------|
| borderWidth | `borderWidth`, `strokeWeight`, `borderWidthTop`, `borderWidthRight`, `borderWidthBottom`, `borderWidthLeft`, `border`, `strokeWidth` |

**Effects/Shadows:**
| Property | Keys Checked |
|----------|-------------|
| boxShadow | `boxShadow`, `shadow`, `boxShadow[0]`, `shadow[0]`, `effects`, `effect`, `dropShadow`, `innerShadow`, `elevation`, `shadowColor`, `shadowOffset`, `shadowRadius`, `shadowSpread` |

### Debug Logging

Debug logging is **enabled by default** in `FigmaComponentService`. When scanning components, check the browser console (Figma DevTools) for:

```
[TokenRef] Found tokens:fill = "ids.color.primary.default" on Button
[TokenData] Node: "Card" (FRAME)
  tokens:borderRadius = "ids.radius.md"
  tokens:boxShadow = "ids.shadow.card"
[Variable] Card.cornerRadius → spacing/radius-md
```

To disable debug logging, set `DEBUG_LOGGING = false` in `figma-component-service.ts`.

### Figma Variable Bindings

For designs using Figma's native Variables (synced from Tokens Studio), we also check `boundVariables` on each node. This catches tokens that were synced to Figma Variables rather than stored as plugin data.

Example: A variable bound to `cornerRadius` with name `spacing/radius-md` will be extracted and matched.
...
1. **Spacing tokens not matching** — If spacing tokens aren't matching, check the console logs for `[TokenRef]` messages. Debug logging is enabled by default in `FigmaComponentService`.

2. **Typography composite tokens** — When Tokens Studio applies a typography token (with fontFamily + fontSize + fontWeight), verify if it stores as one key or multiple.

3. **Figma Variables** — ✅ Now supported! The service checks `boundVariables` on nodes in addition to plugin data.

### Next Steps

1. [x] Run diagnostic tool on components with spacing/dimension tokens applied
2. [x] Document actual keys used by Tokens Studio for spacing (comprehensive list added)
3. [x] Add support for Figma Variables (boundVariables API)
